...
def get_text_from_doi(doi, doi2paper, raise_not_found_error=True):
    ...
    if doi in doi2paper:
        ...
        if (doi2paper[doi]['abstract'] and len(doi2paper[doi]['abstract']) > 10) or (doi2paper[doi]['body_text'] and  len(doi2paper[doi]['body_text']) > 10):
            # text += doi2paper[doi]['metadata']['title'] + '\n' + doi2paper[doi]['abstract'][0]['text']
            text = doi2paper[doi]['title']
            if (doi2paper[doi]['abstract'] and len(doi2paper[doi]['abstract']) > 10):
                text += sep + doi2paper[doi]['abstract']
            if (doi2paper[doi]['body_text'] and len(doi2paper[doi]['body_text']) > 10):
                text += sep + doi2paper[doi]['body_text']
